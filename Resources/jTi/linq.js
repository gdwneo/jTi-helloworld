...
(function(root, undefined) {
	...
	var WhereSelectEnumerable = function(source, predicate, selector) {
		this.prevSource = source;
		this.prevPredicate = predicate;
		// predicate.length always <= 1 or null
		this.prevSelector = selector;
		// selector.length always <= 1
	};
	...
	// for ORM Framework
	var DbSetEnumerable = function(type, context) {
		this.__dataSet = [];
		this.__type = type;
		this.__needSync = true;
		this.__context = context;
		var scorp = this;
		this.getSource = function() {
			if (scorp.__needSync) {
				scorp.__dataSet = scorp.__context.__proxy.get(null, scorp.__type);
				for (var i in scorp.__dataSet) {
					if (!scorp.__dataSet[i].__state) {
						scorp.__dataSet[i].__state = root.jTi.model.state.unchanged;
						//foreignKey refer to the model which it field will have association with others
						//associationKey in the model which will be refer to and it always be primary key
						(new ArrayEnumerable(scorp.__context.__association)).forEach(function(t) {
							// when other models refer to current model
							// eg product.answers
							// when t.model = question
							if (t.model === scorp.__type.name) {
								if (!scorp.__dataSet[i].hasOwnProperty(t.filterProperty)) {
									//define get question.answers
									scorp.__dataSet[i].__defineGetter__(t.filterProperty, function() {
										//associationKey is QuestionID
										//associationValue is Question.QuestionID
										var associationValue = this[t.associationKey], _self = this;
										if (!associationValue) {
											return null;
										}
										// ownerModel is answer, search it
										var data = scorp.__context.set(t.ownerModel).where(function(r) {
											if (!r[t.foreignKey]) {
												return false;
											}
											// if answer.questionid is array, it must be contains
											if (root.jTi.isArray(associationValue)) {
												return r[t.foreignKey].indexOf(_self[t.associationKey]) > -1;
											} else {
												//or answer.questionid equel to question.questionid
												return r[t.foreignKey] === _self[t.associationKey];
											}
										});
										return root.jTi(data.toArray());
									});

									//define set question.answers, answers must be array
									scorp.__dataSet[i].__defineSetter__(t.filterProperty, function(val) {
										for (var index = 0; index < val.length; index++) {
											// if answser.questionid is array
											if (root.jTi.isArray(val[index][t.foreignKey])) {
												// add questionid into answer.questionid
												if (val[index][t.foreignKey].indexOf(this[t.associationKey]) < 0) {
													val[index][t.foreignKey].push(this[t.associationKey]);
												}
											} else {
												// or set the value to answer.questionid
												val[index][t.foreignKey] = this[t.associationKey];
											}
										}
									});
								}
							}
							// when current model will refer to other models
							// eg answer.Question
							// when ownerModel=answer
							if (t.ownerModel === scorp.__type.name) {
								if (!scorp.__dataSet[i].hasOwnProperty(t.name)) {
									// define get answer.Question
									scorp.__dataSet[i].__defineGetter__(t.name, function() {
										// associationValue is answer.QuestionID
										var associationValue = this[t.foreignKey], _self = this;
										if (!associationValue) {
											return null;
										}
										// if answer.QuestionID is array
										if (root.jTi.isArray(associationValue)) {
											// search for Question
											var data = scorp.__context.set(t.model).where(function(r) {
												//return answer.QuestionID has contains Question.QuestionID
												return _self[t.foreignKey].indexOf(r[t.associationKey]) > -1;
											});
											return root.jTi(data.toArray());
										} else {
											//return the one questiion which id equel to answer.QuestionID
											return scorp.__context.set(t.model).singleOrDefault(function(r) {
												return _self[t.foreignKey] === r[t.associationKey];
											});
										}
									});
									scorp.__dataSet[i].__defineSetter__(t.name, function(val) {
										if (root.jTi.isArray(val)) {
										} else {
											val[t.associationKey] = this[t.foreignKey];
										}
									});
								}
							}
				...
				scorp.__needSync = false;
			}
			return scorp.__dataSet;
		};
	};

	DbSetEnumerable.prototype = new ArrayEnumerable();

	DbSetEnumerable.prototype.create = function() {
		var entity = root.jTi.model(this.__type, this.__context.__proxy);
		entity.__state = root.jTi.model.state.deleted;
		return entity;
	};
	DbSetEnumerable.prototype.add = function(entity) {
		this.attach(entity);
		entity.__state = root.jTi.model.state.added;
		this.__dataSet.push(entity);
	};
	DbSetEnumerable.prototype.remove = function(entity) {
		entity.__state = root.jTi.model.state.deleted;
	};
	DbSetEnumerable.prototype.sqlQuery = function(sql, parameters) {
		return this.__context.__proxy.__db.sqlQuery(this.__type, sql, parameters);
	};
	DbSetEnumerable.prototype.attach = function(entity) {
		entity.__proxy = this.__context.__proxy;
		entity.__mapping = this.__type.mapping;
		return entity;
	};
	DbSetEnumerable.prototype.find = function(value) {
		var scope = this;
		return this.singleOrDefault(function(t) {
			return t[scope.__type.mapping.idProperty] === value;
		});
		// return this.where(function(t) {
		// for (var key in keyValues) {
		// if (t[key] != keyValues[key]) {
		// return false;
		// }
		// }
		// return true;
	...
	if ( typeof define === Types.Function && define.amd) {// AMD
		...
		if (root.jTi) {
			// init dbset with type and db or proxy eg:
			// $.dbset(type,dbname) or $.dbset(type,db) or $.dbset(type,proxy)
			root.jTi.dbset = function(type, context) {
				return new DbSetEnumerable(type, context);
			};
		}
	}
})(this);
