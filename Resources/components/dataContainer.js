exports.module = function(param) {
	...
	container.Refresh = function(pathDictionary, updateDate) {
		...
		var index = attributePath.length - 1, parentNode = path[index - 1], dataSource = queryDataSource(index > 0 ? parentNode.data[parentNode.index][attributePath[index]] : jTi.store(attributePath[0]).get(), this.queryProperty);
		...
		//alert( jTi.isFunction(this.onLoadComplete));
